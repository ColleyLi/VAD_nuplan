...
# NOTE: x forward, y sideways in nuPlan/OpenScene
point_cloud_range = [-30.0, -15.0, -2.0, 30.0, 15.0, 2.0]

# for vis
# point_cloud_range = [-32.0, -32.0, -2.0, 32.0, 32.0, 2.0]
voxel_size = [0.15, 0.15, 4]

# upper is wrong, but used for pretrained model
# img_norm_cfg = dict(
#    mean=[103.530, 116.280, 123.675], std=[1.0, 1.0, 1.0], to_rgb=False)
# lower is the correct one
img_norm_cfg = dict(
    mean=[123.675, 116.28, 103.53], std=[58.395, 57.12, 57.375], to_rgb=True)

# TODO: For OpenScene do 8-class detection?
# class_names = [
#     'vehicle', 'pedestrian', 'bicycle', 
#     'traffic_cone', 'barrier', 'czone_sign', 'generic_object', 
#     'ego' # 'background' # TODO: what is background
# ]
# below is what openscene provided, place_holder?
class_names = [
...
model = dict(
    ...
    fut_ts=8, # NOTE: 2Hz, 4s = 8 points
    
    # img_backbone input one frame feature_map: [3, 512, 1024]
    img_backbone=dict(
        type='ResNet',
        depth=50,
        num_stages=4, # total 4 stages
        out_indices=(1, 2, 3), # using latter 3 (2nd ~ 4th) stages' output
        frozen_stages=1, # frozen parameters before and within 1st stage
        norm_cfg=dict(type='BN', requires_grad=False),
        norm_eval=True, # BN layers dont train
        style='pytorch'),
    # img_backbone output one frame feature_map: [512, 1024, 2048]

    img_neck=dict(
        type='FPN',
        in_channels=[512, 1024, 2048], # backbone's output channels
        out_channels=_dim_, # 256
        start_level=0, # the start_level is just a index for in_channels, 0 means process from feature_map of 512
        add_extra_convs='on_output', # feature_map after FPN will be used as extra convs's input
        num_outs=_num_levels_, # 4
        relu_before_extra_convs=True), # relu before extra convs
    # img_neck output one frame feature_map: [256, 256, 256, 256]
    ...
    pts_bbox_head=dict(
        ...
        ego_fut_mode = 4, # NOTE: 4 modes corresponding to 4 commands
        fut_ts=8, # NOTE: 2Hz, 4s = 8 points
        valid_fut_ts=8, # NOTE: 2Hz, 4s = 8 points
        ...
        transformer=dict(
            ...
            encoder=dict(
                ...
                num_points_in_pillar=4, # Z in BEV query
                return_intermediate=False,
                transformerlayers=dict(
                    type='BEVFormerLayer',
                    attn_cfgs=[
                        ...
                        dict(
                            ...
                            deformable_attention=dict(
                                type='MSDeformableAttention3D', # Multi-scale deform attention 3d (from BEVformer?)
                                embed_dims=_dim_,
                                num_points=8,
...
dataset_type = 'VADCustomOpenSceneDataset'
data_root = '/data/zyp/Projects/VAD_open/VAD-main/data/openscene-v1.1/'
        ...
        transforms=[
            ...
            dict(type='CustomCollect3D',\
                 keys=['gt_bboxes_3d', 'gt_labels_3d', 'img', 'fut_valid_flag',
                       ...
                       'ego_lcf_feat', 'gt_attr_labels'])]) # 'points' and 'fut_valid_flag' are unique from train
]
...
data = dict(
    samples_per_gpu=1,
    workers_per_gpu=4,
    # TODO: we only use mini for train and validation for now
    train=dict(
            type=dataset_type,
            data_root=data_root,
            # ann_file=data_root + 'navsim_dataset-mini_infos_temporal_train.pkl',
            ann_file=data_root + 'gyf_navsim_mini_train.pkl',
            ...
            # map_ann_file=data_root + 'fake_openscene_map_anns_val.json',
    ...
    val=dict(type=dataset_type,
             data_root=data_root,
            #  ann_file=data_root + 'navsim_dataset-mini_infos_temporal_val.pkl',
             ann_file=data_root + 'gyf_navsim_mini_val.pkl',
             ...
             samples_per_gpu=1,
             map_classes=map_classes,
             map_ann_file=data_root + 'openscene_map_anns_val.json',
             map_fixed_ptsnum_per_line=map_fixed_ptsnum_per_gt_line,
             map_eval_use_same_gt_sample_num_flag=map_eval_use_same_gt_sample_num_flag,
             use_pkl_result=True,
             custom_eval_version='vad_openscene_planning_cvpr_2024'),
