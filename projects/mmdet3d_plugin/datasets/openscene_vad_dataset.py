...
from .vad_custom_nuscenes_eval import NuScenesEval_custom # TODO: needs to be updated
...
from projects.mmdet3d_plugin.core.bbox.structures.nuscenes_box import CustomNuscenesBox # pass
...
# from nuplan.common.maps.nuplan_map import NuPlanMap
...
from nuplan.common.actor_state.tracked_objects_types import TrackedObjectType, AGENT_TYPES, STATIC_OBJECT_TYPES
...
class VectorizedLocalMap(object):
    ...
    CLASS2LABEL = {
        'lane_connectors': 0,
        'lanes_polygons': 0, # 'boundaries': 0, # TODO: is it right?
        'ped_crossing': 1,
        'contours': 2,
        'others': -1
    }
    def __init__(self,
                ...
                #  line_classes=['road_divider', 'lane_divider'],
                 line_classes=['lanes_polygons', 'lane_connectors'], # TODO: is it right? ,  
                #  ped_crossing_classes=['ped_crossing'],
                 ped_crossing_classes=['crosswalks'],
                #  contour_classes=['road_segment', 'lane'],
                 contour_classes=['road_segments'], # TODO: is it right? 
                 sample_dist=1,
                 num_samples=250,
                 padding=False,
        ...
        self.MAPS = ['sg-one-north', 'us-ma-boston',
        ...
        assert sorted(self.MAPS) == sorted(self.maps_db.get_locations()), "Missing Locations"
        self.nuplan_maps = {}
        self.map_explorer = {}
        for loc in self.MAPS:
            self.nuplan_maps[loc] = NuPlanMapWrapper(maps_db=self.maps_db, map_name=loc)
            self.map_explorer[loc] = NuPlanMapExplorer(self.nuplan_maps[loc])
    ...
    def gen_vectorized_samples(self, location, lidar2global_translation, lidar2global_rotation):
        ...
        patch_box = (map_pose[0], map_pose[1], self.patch_size[0], self.patch_size[1]) # a 
        ...
        for vec_class in self.vec_classes:
            if vec_class == 'divider':
                '''
                new version, from navsim
                '''
                for layer_name in self.line_classes:
                    layer_df = self.map_explorer[location].map_api.load_vector_layer(layer_name)
                    patch = self.map_explorer[location].map_api.get_patch_coord(patch_box, patch_angle)

                    map_object_ids = layer_df[layer_df['geometry'].intersects(patch)]['fid']
                    layer = SemanticMapLayer.LANE if layer_name == 'lanes_polygons' else SemanticMapLayer.LANE_CONNECTOR
                    map_object_list = [self.map_explorer[location].map_api.get_map_object(map_object_id, layer) for map_object_id in map_object_ids]
                    ...
                    for map_object in map_object_list:
                        ...
                        # there are some line_connecters that are reversed --- turns out it wouldnt
                        # if layer_name == 'lane_connectors':
                        #     should_reverse = False if check_lane_connecter_heading(map_object, patch, patch_box, patch_angle) else True
                        #     if should_reverse:
                        #         line = line.reverse()
                        #         break
                        if not line.is_empty:
                            new_line = trans_and_rotate_line(line, patch, patch_box, patch_angle)
                            if new_line:
                                line_list.append(new_line)
                                line_object_list.append(map_object)
                    line_instances_dict[layer_name] = line_list
                    
                    # lane boundary lines
                    if layer_name == 'lanes_polygons':
                        for map_object in map_object_list:
                            left_lane, right_lane = map_object.adjacent_edges
                            if left_lane is None: # no left lane, extract left boundary
                                left_boundary = map_object.left_boundary.linestring
                                if not left_boundary.is_empty:
                                    new_left_boundary = trans_and_rotate_line(left_boundary, patch, patch_box, patch_angle)
                                    if new_left_boundary:
                                        boundary_list.append(new_left_boundary)
                            if right_lane is None: # no right lane, extract right boundary
                                right_boundary = map_object.right_boundary.linestring
                                if not right_boundary.is_empty:
                                    new_right_boundary = trans_and_rotate_line(right_boundary, patch, patch_box, patch_angle)
                                    if new_right_boundary:
                                        boundary_list.append(new_right_boundary)

                    elif layer_name == 'lane_connectors': # currently Nuplan don't have a way of telling if two lane connectors are adiacent
                        for map_object in line_object_list:
                            outgoing_object, incoming_object = map_object.outgoing_edges[0], map_object.incoming_edges[0]
                            is_left = (outgoing_object.adjacent_edges[0] is None) and (incoming_object.adjacent_edges[0] is None)
                            is_right = (outgoing_object.adjacent_edges[1] is None) and (incoming_object.adjacent_edges[1] is None)
                            if is_left:
                                left_boundary = map_object.left_boundary.linestring
                                if not left_boundary.is_empty:
                                    new_left_boundary = trans_and_rotate_line(left_boundary, patch, patch_box, patch_angle)
                ...
                # make potential MultiLineString instances become LineString instances
                for layer_name, line_list in line_instances_dict.items():
                    line_instances_dict[layer_name] = self._one_type_line_geom_to_instances(line_list)
                boundary_list = self._one_type_line_geom_to_instances(boundary_list)
                
                # add to vectors list, for both centerlines and boundaries
                for line_type, instances in line_instances_dict.items():
                    for instance in instances:
                        assert isinstance(instance, LineString)
                        vectors.append((instance, self.CLASS2LABEL.get(line_type, -1)))
                for boundary in boundary_list:
                    assert isinstance(boundary, LineString)
                    vectors.append((boundary, self.CLASS2LABEL.get('contours', -1)))

                '''
                old version, modified from nuscenes
                '''
                # line_geom = self.map_explorer[location].get_map_geom(patch_box, patch_angle, layer_names=self.line_classes)
                # line_instances_dict = self.line_geoms_to_instances(line_geom)     
                # for line_type, instances in line_instances_dict.items():
                #     for instance in instances:
                #         vectors.append((instance, self.CLASS2LABEL.get(line_type, -1)))

            elif vec_class == 'ped_crossing': # ['crosswalks']
                ped_geom = self.map_explorer[location].get_map_geom(patch_box, patch_angle, layer_names=self.ped_crossing_classes)
                ped_instance_list = self.ped_poly_geoms_to_instances(ped_geom)
                for instance in ped_instance_list:
                    vectors.append((instance, self.CLASS2LABEL.get('ped_crossing', -1)))
            elif vec_class == 'boundary': # ['road_segment', 'lanes_polygons']
                pass
            #     polygon_geom = self.map_explorer[location].get_map_geom(patch_box, patch_angle, layer_names=self.polygon_classes)
    ...
    def custom_poly_geoms_to_instances(self, polygon_geom):
        union_segments = ops.unary_union(polygon_geom)
    ...
    # def custom_line_geoms_to_instances(self, line_geom):
    #     line_instances_dict = dict()
    #     for line_type, a_type_of_lines in line_geom:
    #         one_type_instances = self._one_type_line_geom_to_instances(a_type_of_lines)
    #         line_instances_dict[line_type] = one_type_instances
    ...
    #     if layer_name not in self.map_explorer[location].map_api.vector_polygon_layers:
    #         raise ValueError('{} is not a polygonal layer'.format(layer_name))
    ...
    #     patch = self.map_explorer[location].map_api.get_patch_coord(patch_box, patch_angle) # map_api function pass

    #     records = getattr(self.map_explorer[location].map_api, layer_name)

    #     polygon_list = []
    #     # if layer_name == 'drivable_area':
    #     if layer_name in ['road_segments', 'lanes_polygons', 'intersections', 'generic_drivable_areas', 'carpark_areas']: # TODO: nuPlan define "drivable_area"
    ...
    #     if layer_name not in self.map_explorer[location].map_api.vector_line_layers:
    #         raise ValueError("{} is not a line layer".format(layer_name))
...
class v1CustomDetectionConfig:
    ...
    def __init__(self,
        ...
        # assert set(class_range_x.keys()) == set([i.fullname for i in list(AGENT_TYPES)] + [i.fullname for i in list(STATIC_OBJECT_TYPES)]), "Class count mismatch."
...
class VADCustomOpenSceneDataset(NuPlanDataset):
    r"""Custom OpenScene/Nuplan Dataset.
    """
    MAPCLASSES = ('divider',)
    def __init__(
        self,
        ...
        custom_eval_version='vad_openscene_planning_cvpr_2024',
        *args,
        **kwargs
    ):
    ...
    def vectormap_pipeline(self, example, input_dict):
        ...
        '''
        # import pdb;pdb.set_trace()
        # print("lidar2ego_rotation", input_dict['lidar2ego_rotation'])
        lidar2ego = np.eye(4)
        ...
        # print("lidar2global_translation_convert", lidar2global_translation)
        # print("lidar2global_rotation_convert", lidar2global_rotation)
        # print("input_dict", input_dict.keys())
        # print("input_dict", input_dict["scene_token"])
        # print("input_dict", input_dict["frame_idx"])
    ...
    def prepare_train_data(self, index):
        ...
        # temporal aug, random filter out one frame from previous 4 frames
        prev_indexs_list = list(range(index-self.queue_length, index))
        random.shuffle(prev_indexs_list)
        prev_indexs_list = sorted(prev_indexs_list[1:], reverse=True) # prev 3 + current 1 = 4 = queue_length
        ##

        # NOTE: the meaning of frame_idx and scene_token is different from the original dataset
        input_dict = self.get_data_info(index)
        if input_dict is None:
            return None
        frame_idx = input_dict['frame_idx']
        scene_token = input_dict['scene_token']        
        self.pre_pipeline(input_dict) # add some new items
        
        example = self.pipeline(input_dict) # which is 'train_pipeline' in the config, a series of augmentation and transition of the original images
        example = self.vectormap_pipeline(example, input_dict) # add map labels info into example
    ...
    def get_ann_info(self, index):
        ...
        gt_names_3d = info['gt_names'][mask] # object's names
        gt_labels_3d = []
        
        # TODO: labels are indexes?
    ...
    def get_data_info(self, index):
        ...
        input_dict = dict(
            ...
            prev_idx=info['prev'],
            next_idx=info['next'],
            # NOTE: here we changed the frame_idx to big_scene_frame_idx
            frame_idx=info['big_scene_frame_idx'], # NOTE: custom key
            # NOTE: here we changed the scene_token to big_scene_token
            scene_token=info['big_scene_token'], 
            # NOTE: below 2 lines are the original frame_idx and scene_token
            ori_frame_idx=info['frame_idx'],
            ori_scene_token=info['scene_token'],
        ...
        annos = self.get_ann_info(index) # keys: 'gt_bboxes_3d', 'gt_labels_3d', 'gt_names', 'attr_labels'
        input_dict['ann_info'] = annos
    ...
    def _format_bbox(self, results, jsonfile_prefix=None, score_thresh=0.2):
        ...
        for sample_id, det in enumerate(mmcv.track_iter_progress(results)):
            ...
            # rotate and translate the box to global frame
            boxes = lidar_nusc_box_to_global(self.data_infos[sample_id], boxes,
                                             det_mapped_class_names,
                                             self.custom_eval_detection_configs,
                                             self.eval_version)

            for i, box in enumerate(boxes): # score_thresh is 0.2
                ...
                if np.sqrt(box.velocity[0]**2 + box.velocity[1]**2) > 0.2:
                    ...
                    # elif name in ['bicycle', 'motorcycle']:
                        ...
                        attr = NuPlanDataset.DefaultAttribute[name]
                else:
    ...
    def evaluate(self,
        ...
        motion_cls_names = ['vehicle', 'pedestrian']
...
def output_to_nusc_box(detection): # pass
...
def lidar_nusc_box_to_global(info,
                             ...
                             classes,
                             eval_configs,
                             eval_version='detection_cvpr_2019'): # pass
...
def output_to_vecs(detection): # pass
...
# line boundary
def cluster_road_segments(start, heading, connectors_in_different_road_segments):
    for rec_heading in connectors_in_different_road_segments.keys():
        # heading judgement
        if abs(heading - rec_heading) > 90:
            continue
        # distance judgement
        # distances = [np.sqrt((linstring.xy[0][0] - start[0])**2 + (linstring.xy[1][0] - start[1])**2) 
        #             for linstring in connectors_in_different_road_segments[rec_heading]]
        # if min(distances) > 10:
        #     continue
        return rec_heading
    return None

def trans_and_rotate_line(line, patch, patch_box, patch_angle):
    new_line = line.intersection(patch)
    if not new_line.is_empty:
        new_line = affinity.rotate(new_line, -patch_angle, origin=(patch_box[0], patch_box[1]), use_radians=False)
        new_line = affinity.affine_transform(new_line, [1.0, 0.0, 0.0, 1.0, -patch_box[0], -patch_box[1]])
        return new_line
    return None

def check_lane_connecter_heading(map_object, patch, patch_box, patch_angle):
    def get_heading(p1, p2):
        slope = (p2[1] - p1[1])/(p2[0] - p1[0])
        heading = np.degrees(np.arctan(slope)) % 360
        return heading

    outgoing_object, incoming_object = map_object.outgoing_edges[0], map_object.incoming_edges[0]
    outgoing_line, incoming_line = outgoing_object.baseline_path.linestring, incoming_object.baseline_path.linestring
    lane_connecter_line = map_object.baseline_path.linestring
    
    outgoing_good, incoming_good = True, True
    if not lane_connecter_line.is_empty:
        lane_connecter_line = trans_and_rotate_line(lane_connecter_line, patch, patch_box, patch_angle)
        if not lane_connecter_line.is_empty:
            if not outgoing_line.is_empty:
                outgoing_line = trans_and_rotate_line(outgoing_line, patch, patch_box, patch_angle)
                if outgoing_line:
                    outgoing_start_heading = get_heading((outgoing_line.xy[0][0], outgoing_line.xy[1][0]), (outgoing_line.xy[0][1], outgoing_line.xy[1][1]))
                    lane_connecter_end_heading = get_heading((lane_connecter_line.xy[0][-2], lane_connecter_line.xy[1][-2]), (lane_connecter_line.xy[0][-1], lane_connecter_line.xy[1][-1]))
                    outgoing_good = abs(outgoing_start_heading - lane_connecter_end_heading) < 90
            if not incoming_line.is_empty:
                incoming_line = trans_and_rotate_line(incoming_line, patch, patch_box, patch_angle)
                if incoming_line:
                    incoming_end_heading = get_heading((incoming_line.xy[0][-2], incoming_line.xy[1][-2]), (incoming_line.xy[0][-1], incoming_line.xy[1][-1]))
                    lane_connecter_start_heading = get_heading((lane_connecter_line.xy[0][0], lane_connecter_line.xy[1][0]), (lane_connecter_line.xy[0][1], lane_connecter_line.xy[1][1]))
                    incoming_good = abs(incoming_end_heading - lane_connecter_start_heading) < 90

    return outgoing_good and incoming_good
