...
class VectorizedLocalMap(object):
    ...
    def __init__(self,
                ...
                #  line_classes=['road_divider', 'lane_divider'],
                 line_classes=['lane_connectors', 'lanes_polygons'], # TODO: is it right? , 'lanes_polygons',
                #  ped_crossing_classes=['ped_crossing'],
                 ped_crossing_classes=['crosswalks'],
                #  contour_classes=['road_segment', 'lane'],
                 contour_classes=['road_segments', 'lanes_polygons'], # TODO: is it right? 
                 sample_dist=1,
                 num_samples=250,
                 padding=False,
    ...
    def gen_vectorized_samples(self, location, lidar2global_translation, lidar2global_rotation):
        ...
        # get different map layers
        line_instances_dict = {}
        for vec_class in self.vec_classes:
            if vec_class == 'divider': # ['lane_connectors', 'boundaries']
                map_geom = []
                for layer_name in self.line_classes:
                    ...
                    # layer_geom = self.map_explorer[location].map_api.get_layer_line(patch_box, patch_angle, layer_name)
                    # map_geom.append((layer_name, layer_geom))
                    # for item in map_geom:
                    #     key = item[0]
                    #     value = item[1]    
                    #     one_type_instances = self._one_type_line_geom_to_instances(value)
                    #     line_instances_dict[key] = one_type_instances

                # line_geom = self.map_explorer[location].get_map_geom(patch_box, patch_angle, layer_names=self.line_classes)
                # for item in line_geom:
                #     key = item[0]
                #     value = item[1]    
                #     if key == 'lane_connectors':
                #         one_type_instances = self._one_type_line_geom_to_instances(value)
                #     elif key == 'lanes_polygons':
                #         one_type_instances = self.custom_poly_geoms_to_instances(value)
                #     line_instances_dict[key] = one_type_instances
