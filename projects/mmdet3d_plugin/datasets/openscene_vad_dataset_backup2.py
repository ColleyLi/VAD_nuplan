...
class VectorizedLocalMap(object):
    ...
    def __init__(self,
                ...
                #  line_classes=['road_divider', 'lane_divider'],
                 line_classes=['lane_connectors'], # TODO: is it right? , 'lanes_polygons', 
                ...
                for layer_name in self.line_classes:
                    ...
                    for map_object in map_object_list:
                        ...
                        if not new_line.is_empty:
                            ...
                            line_map_object_list.append(map_object)

                    line_instances_dict[layer_name] = line_list

                    # currently Nuplan don't have a way of telling if two lane connectors are adiacent
                    if layer_name == 'lane_connectors': 
                        # cluster lines
                        connectors_in_different_road_segments = {} # {heading: list(LineString)}
                        for index, line in enumerate(line_list):
                            x, y = line.xy
                            start, end = np.array([x[0], y[0]]), np.array([x[-1], y[-1]])
                            slope = (end[1] - start[1])/(end[0] - start[0])
                            heading = np.degrees(np.arctan(slope)) % 360
                            rec_heading = cluster_road_segments(start, heading, connectors_in_different_road_segments)
                            if rec_heading:
                                connectors_in_different_road_segments[rec_heading].append(line_map_object_list[index])
                            else:
                                connectors_in_different_road_segments[heading] = [line_map_object_list[index]]

                        # find line boundries
                        for cluster_heading, map_object_list in connectors_in_different_road_segments.items():
                            first_map_object = map_object_list[0]
                            first_line = first_map_object.baseline_path.linestring
                            origin = (first_line.xy[0][0], first_line.xy[1][0], cluster_heading)
                            min_y, left_object = 0., first_map_object
                            max_y, right_object = 0., first_map_object
                            for map_object in map_object_list:
                                line = map_object.baseline_path.linestring
                                new_line = affinity.rotate(line, -origin[2], origin=(origin[0], origin[1]), use_radians=False)
                                new_line = affinity.affine_transform(new_line, [1.0, 0.0, 0.0, 1.0, -origin[0], -origin[1]])
                                # NOTE: y is inverted
                                if new_line.xy[1][0] < min_y:
                                    min_y, right_object = new_line.xy[1][0], map_object
                                if new_line.xy[1][0] > max_y:
                                    max_y, left_object = new_line.xy[1][0], map_object
                            # get boundary lines for each bordar
                            left_boundary = left_object.left_boundary.linestring
                            new_left_boundary = left_boundary.intersection(patch)
                            if not new_left_boundary.is_empty:
                                new_left_boundary = affinity.rotate(new_left_boundary, -patch_angle, origin=(patch_box[0], patch_box[1]), use_radians=False)
                                new_left_boundary = affinity.affine_transform(new_left_boundary, [1.0, 0.0, 0.0, 1.0, -patch_box[0], -patch_box[1]])
                                boundary_list.append(new_left_boundary)

                            # right_boundary = right_object.right_boundary.linestring
                            # new_right_boundary = right_boundary.intersection(patch)
                            # if not new_right_boundary.is_empty:
                            #     new_right_boundary = affinity.rotate(new_right_boundary, -patch_angle, origin=(patch_box[0], patch_box[1]), use_radians=False)
                            #     new_right_boundary = affinity.affine_transform(new_right_boundary, [1.0, 0.0, 0.0, 1.0, -patch_box[0], -patch_box[1]])
                            #     boundary_list.append(new_right_boundary)
                            



                    elif layer_name == 'lanes_polygons':
                        for map_object in map_object_list:
