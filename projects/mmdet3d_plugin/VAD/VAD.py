...
class VAD(MVXTwoStageDetector):
    ...
    def __init__(self,
        ...
        # grid mask is to augment images by structurely deleting a part of the image, prob from 0% to 70% during training along with epochs growing. 
        self.grid_mask = GridMask(
            True, True, rotate=1, offset=False, ratio=0.5, mode=1, prob=0.7)
        self.use_grid_mask = use_grid_mask
        self.fp16_enabled = False
        self.fut_ts = fut_ts # future time steps? 2Hz, 6 steps = 3s
        self.fut_mode = fut_mode # future modes of prediction, 6 modes
        self.valid_fut_ts = pts_bbox_head['valid_fut_ts'] # 6
    ...
    # similar to BEVformer, extract features of images
    def extract_img_feat(self, img, img_metas, len_queue=None): 
        ...
        if img is not None:
            ...
            if self.use_grid_mask: # config default True
                img = self.grid_mask(img)

            img_feats = self.img_backbone(img) # ResNet50
            if isinstance(img_feats, dict):
                img_feats = list(img_feats.values())
        else:
            return None
        if self.with_img_neck: # FPN
            img_feats = self.img_neck(img_feats) # FPN, results in 4 feats

        img_feats_reshaped = []
        for img_feat in img_feats:
            BN, C, H, W = img_feat.size()
             # "len_queue" means multiple timesteps
    ...
    def obtain_history_bev(self, imgs_queue, img_metas_list):
        ...
        with torch.no_grad():
            ...
            img_feats_list = self.extract_feat(img=imgs_queue, len_queue=len_queue) # [bs, len_queue(no current), cam, C, H, W]
            for i in range(len_queue): # for every previous frame
                img_metas = [each[i] for each in img_metas_list] # meta info in this frame
                # img_feats = self.extract_feat(img=img, img_metas=img_metas)
                img_feats = [each_scale[:, i] for each_scale in img_feats_list] # img fetures in this frame
                prev_bev = self.pts_bbox_head(
                    img_feats, img_metas, prev_bev, only_bev=True) # bev in this frame
            self.train()
            return prev_bev
    ...
    def forward_train(self,
        ...
        img_metas = [each[len_queue-1] for each in img_metas] # current meta info
        img_feats = self.extract_feat(img=img, img_metas=img_metas) # current img features
