...
class VADPerceptionTransformer(BaseModule):
    ...
    def get_bev_features(
            self,
            mlvl_feats, # list of 4 elements, each: (bs, num_cam, C=256, H, W)
            bev_queries,
            bev_h,
            bev_w,
        ...
        # bev_queries and bev_pos share the same shape, (bev_h*bev_w, bs, embed_dims)
        bev_queries = bev_queries.unsqueeze(1).repeat(1, bs, 1) # 'repeat' means queries initialize the same way for every bev
        bev_pos = bev_pos.flatten(2).permute(2, 0, 1)

        # obtain rotation angle and shift in bev with ego motion
        # delta of position and angle in global frame
        delta_x = np.array([each['can_bus'][0]
                           for each in kwargs['img_metas']])
        delta_y = np.array([each['can_bus'][1]
                           for each in kwargs['img_metas']])
        ego_angle = np.array(
            [each['can_bus'][-2] / np.pi * 180 for each in kwargs['img_metas']]) # rad to degree
        # h and w length in real world for 1 grid in bev
        ...
        # how many grids should it move in bev
            ...
            if self.rotate_prev_bev:
                for i in range(bs):
                    ...
                    tmp_prev_bev = rotate(tmp_prev_bev, rotation_angle,
                                          center=self.rotate_center) # TODO: why rotate_center is fixed to 100,100 ?
        ...
        # feature augmentation, add cams embeds and level embeds
        feat_flatten = []
        spatial_shapes = []
        # self.cams_embeds is a learnable parameter, shape: (num_cams, embed_dims)
        ...
        feat_flatten = torch.cat(feat_flatten, 2) # 6, bs, 4*h*w(concat 4 level features), 256
        spatial_shapes = torch.as_tensor(
            spatial_shapes, dtype=torch.long, device=bev_pos.device) # shapes of 4 level features
        # record start index for different level of features
        ...
        bev_embed = self.encoder(
            ...
            feat_flatten, # key
            feat_flatten, # value
            bev_h=bev_h, # 200/100 (base/tiny setting)
            bev_w=bev_w, # 200/100 (base/tiny setting)
            bev_pos=bev_pos,
        ...
        return bev_embed # (bs, bev_h*bev_w, 256)
    ...
    def forward(self,
        ...
        bev_embed = self.get_bev_features(
            ...
            bev_pos=bev_pos,
            prev_bev=prev_bev,
            **kwargs) # kwargs only contains 'img_metas'
