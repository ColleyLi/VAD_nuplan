...
class VADHead(DETRHead):
    ...
    def __init__(self,
        ...
        self.real_w = self.pc_range[3] - self.pc_range[0] # width 30m
        self.real_h = self.pc_range[4] - self.pc_range[1] # hight 60m
    ...
    def forward(self,
                mlvl_feats, # camera images featurs (Multi-level features that may have different resolutions, from backbone+FPN networks) 
                img_metas,
                prev_bev=None,
                only_bev=False,
                ego_his_trajs=None,
        ...
        # TODO: object_query_embeds and map_query_embeds used for what?
        object_query_embeds = self.query_embedding.weight.to(dtype) # get query embeds' weights, TODO: why taking it out?
        
        if self.map_query_embed_type == 'all_pts':
            map_query_embeds = self.map_query_embedding.weight.to(dtype)
        elif self.map_query_embed_type == 'instance_pts':
            map_pts_embeds = self.map_pts_embedding.weight.unsqueeze(0) # bs, 20, 512
            map_instance_embeds = self.map_instance_embedding.weight.unsqueeze(1) # 100, bs, 512
            map_query_embeds = (map_pts_embeds + map_instance_embeds).flatten(0, 1).to(dtype) # 2000, 512 # TODO: bs?

        # get BEV query/embeds' weights, TODO: why taking it out?
        bev_queries = self.bev_embedding.weight.to(dtype) # bev_h*bev_w, 256
        
        # BEV position encoding, TODO: encoding zeros?
        bev_mask = torch.zeros((bs, self.bev_h, self.bev_w),
                               device=bev_queries.device).to(dtype)
        bev_pos = self.positional_encoding(bev_mask).to(dtype) # bs, 256, bev_h, bev_w
