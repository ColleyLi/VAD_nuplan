...
def _fill_navsim_trainval_infos(root_path,
    ...
    # 仅仅挑选 300 个数据
    for train_log in train_logs[:10]:
        ...
        for index in range(tokens_num):
            ...
            # obtain annotation
            if not test:
                annotation = cur_token['anns']
                cur_anns = cur_token['anns']
                ...
                for i in range(num_box):
                    names.append(annotation["gt_names"][i])
                ...
                for i in range(num_box):
                    ...
                    # TODO 暂时没有在 metadata 里面找到对应的category_name
                    ...
                    # 计算agent 未来的轨迹 经过讨论发现我们需要进行坐标转化
                    ...
                    def pose_from_matrix(transform_matrix):
                        ...
                        return np.array([transform_matrix[0, 2], transform_matrix[1, 2], heading])
                    def matrix_from_pose(pose):
                        # print("pose", type(pose))
                        """
                        Converts a 2D pose to a 3x3 transformation matrix

                        :param pose: 2D pose numpy array [x, y, yaw]
                    ...
                    def relative_to_absolute_poses(origin_pose, relative_pose):
                        """
                        ...
                        """
                        # relative_transform = np.array([matrix_from_pose(relative_pose)])
                        relative_transform = matrix_from_pose(relative_pose)
                        origin_transform = matrix_from_pose(origin_pose)
                        absolute_transform = origin_transform @ relative_transform
                        absolute_poses = pose_from_matrix(absolute_transform)

                        return absolute_poses
                    
                    def absolute_to_relative_poses(origin_pose, absolute_pose):
                        """
                        ...
                        """
                        absolute_transform = np.array(matrix_from_pose(absolute_pose))
                        origin_transform = np.linalg.inv(matrix_from_pose(origin_pose))
                        relative_transform = origin_transform @ absolute_transform
                        relative_pos = pose_from_matrix(relative_transform)

                        return relative_pos

                    
                    ego_translation = info["ego2global_translation"]
                    ego_quaternion = Quaternion(*info["ego2global_rotation"])
                    cur_global_ego_pose = np.array(
                        [ego_translation[0], ego_translation[1], ego_quaternion.yaw_pitch_roll[0]],
                        dtype=np.float64,
                    )
                            
                    current_index = index
                    cur_box_track_token = annotation["track_tokens"][i]
                    for j in range(fut_ts):
                        if current_index + 1 < tokens_num and openscene_data[current_index + 1]['frame_idx'] != 0: 
                            # 存在当前 scene_token 的下一个token
                            # 这里有个问题就是当前帧和下一帧的 box 数量会变化
                            next_anns = openscene_data[current_index + 1]['anns']
                            
                            # 得到下一帧对应的 ego 绝对坐标位姿
                            next_ego_translation = openscene_data[current_index + 1]["ego2global_translation"]
                            next_ego_quaternion = Quaternion(*openscene_data[current_index + 1]["ego2global_rotation"])
                            next_global_ego_pose = np.array(
                                [next_ego_translation[0], next_ego_translation[1], next_ego_quaternion.yaw_pitch_roll[0]],
                                dtype=np.float64,
                            )
                            
                            # 根据 track_token 来找寻同样的box
                            next_box_num = len(next_anns["gt_boxes"])
                            # 只有在下一时刻的 box 不是 0 的时候才会进行下面的操作
                            if next_box_num != 0:
                                for k in range(next_box_num):
                                    if cur_box_track_token == next_anns["track_tokens"][k]:
                                        break
                                if k != next_box_num - 1:
                                    agent_next_box = next_anns['gt_boxes'][k]
                                    # TODO 这里应该是 navsim 本身的 boxes 就是相对于当前帧的坐标
                                    
                                 
                                    next_agent_abs_pos =  relative_to_absolute_poses(next_global_ego_pose, agent_next_box[:3])
                                    next_agent_rela_cur_ego_pos = absolute_to_relative_poses(cur_global_ego_pose, next_agent_abs_pos)
                                    agent_next_box = next_agent_rela_cur_ego_pos
                                    
                                    gt_fut_trajs[i,j] = (agent_next_box[:2]-cur_box[:2])
                                    gt_fut_masks[i,j] = 1
                                    # TODO 这里的yaw 我直接认为是box的heading了 不知道会不会有问题
                                    gt_fut_yaw[i,j] = agent_next_box[2] - cur_box[2]
                                    
                                    cur_box = agent_next_box
                                    current_index += 1
                                else:
                                    gt_fut_trajs[i, j:] = 0
                ...
                for i in range(his_ts, -1, -1):
                    if cur_token is not None:
                        ...
                        # TODO 这里有 bug 本来以为直接调用就可以了但是这里的 None 仅仅在pkl最后出现 已经写了别的命令解决了
                        if current_index + 1 < tokens_num and openscene_data[current_index + 1]['scene_token'] == openscene_data[current_index]['scene_token']:
                            has_next = True
                        else:
                            has_next = False
                        
                        if current_index - 1 > 0 and openscene_data[current_index - 1]['scene_token'] == openscene_data[current_index]['scene_token']:
                            has_prev = True
                        else:
                            has_prev = False
                ...
                if info["token"] == "b214f8e744075e96":
                    print("info['gt_agent_fut_trajs']",info['gt_agent_fut_trajs'])
                # print("info['gt_agent_fut_trajs']",info['gt_agent_fut_trajs'])
                # print("info['gt_agent_lcf_feat']",info['gt_agent_lcf_feat'])
...
def obtain_sensor2top_cam(cur_token, 
    ...
    # print("fx_l2e_r_s_mat", fx_l2e_r_s_mat)
    # print("sensor2lidar_rotation", sweep["sensor2lidar_rotation"])
...
def obtain_sensor2top_lidar(
    ...
    sweep = {
        ...
        'sensor2ego_translation': prev_token['lidar2ego_translation'],
        'sensor2ego_rotation': prev_token['lidar2ego_rotation'],
...
parser.add_argument(
    '--out_dir',
    type=str,
    default='/data/zyp/Projects/VAD_open/VAD-main/tools/data_converter/navsim_vad_converter_pkl',
    # required='False',
