...
def navsim_data_prep(logs_path,
    ...
    # def convert_to_navsimscenario(scene_dict: Dict[str, Any]):
    ...
    #     )
    #     ego_state=scenario.get_ego_state_at_iteration(3)

    #     return ego_state
    
    # for idx, scene_dict in enumerate(scene_loader.scene_frames_dicts.values()):
    #     file_cache_metadata=convert_to_navsimscenario(scene_dict)
    
    # save train pkl
    scene_filter = SceneFilter()
    scene_loader = SceneLoader(
        Path(os.getenv("OPENSCENE_DATA_ROOT")) / f"navsim_logs/{dataset_type}",
        Path(os.getenv("OPENSCENE_DATA_ROOT")) / f"sensor_blobs/{dataset_type}",
        scene_filter,
    ...
    if dataset_type == 'mini':
        with open('/data/zyp/Projects/VAD_open/navsim_gyf/navsim/planning/script/config/common/scene_filter/warmup_test_e2e.yaml', 'r') as file:
            test_data = yaml.safe_load(file)
        scene_filter = SceneFilter(log_names=test_data['log_names'], tokens=test_data['tokens'], has_route=True)
    elif dataset_type == 'trainval':
        with open('/data/zyp/Projects/VAD_open/navsim_gyf/navsim/planning/script/config/common/scene_filter/private_test_e2e.yaml', 'r') as file:
            test_data = yaml.safe_load(file)
        scene_filter = SceneFilter(log_names=test_data['log_names'], tokens=test_data['tokens'], has_route=False)
    ...
    val_openscene_infos, val_key_tokens = fill_navsim_trainval_infos(scene_loader, logs_path, sensor_path, train=False, max_sweeps=max_sweeps)
...
def fill_navsim_trainval_infos(scene_loader,
    ...
    if train:
        ...
        key_tokens = scene_loader._scene_filter.tokens
    ...
    # find BIG scenes
    ## for val/test, only key_tokens and their former tokens (to form ego history feature) are used
    if not train:        
        big_scene_dict = {}
        big_scene_index = 0
        big_scene_tokens = []
        for token in key_tokens:
            raw_data = raw_data_dict[token]
            big_scene_tokens.append(token)
            # get 3 former tokens, plus current token, 4 in total
            for _ in range(3): 
                if raw_data['sample_prev'] in raw_data_dict.keys():
                    prev_token = raw_data['sample_prev']
                    big_scene_tokens.insert(0, prev_token)
            # add to big_scene_dict
            big_scene_dict['big_scene_{}'.format(big_scene_index)] = big_scene_tokens
            big_scene_index += 1
            big_scene_tokens = []
            
    ## for train, all tokens are processed
    else:
    ...
    for big_scene, big_scene_tokens_list in mmcv.track_iter_progress(big_scene_dict.items()):
        ...
        for big_scene_frame_idx, big_scene_token in enumerate(tqdm(big_scene_tokens_list, desc='Processing big scene: {}'.format(big_scene))): 
            ...
            # TODO: seems like vx and vy are velocity in Lidar frame? according to the code in nuscenes_converter. yet here they are in global frame
            ego_yaw = ego_pose[2]
            if big_scene_frame_idx - 1 >= 0:
            ...
            elif big_scene_frame_idx + 1 < len(big_scene_tokens_list) - 1:
                ...
                ego_w = 0
                ego_v = np.linalg.norm(ego_status['ego_velocity'])
                ego_vx, ego_vy = ego_v * math.cos(ego_yaw), ego_v * math.sin(ego_yaw)
            ...
            ego_lcf_feat[2:4] = ego_status['ego_acceleration'] # TODO: which frame?
            ...
            for camera_type, camera_info in camera_data.items():
                ...
                want_info = {
                            ...
                            'sensor2ego_translation': camera_info['sensor2lidar_translation'], # sensor2ego_translation,
                            'sensor2ego_rotation': Quaternion(matrix=camera_info['sensor2lidar_rotation']), # sensor2ego_rotation,
                            'ego2global_translation': ego2global_translation,
                            'ego2global_rotation': ego2global_rotation,
