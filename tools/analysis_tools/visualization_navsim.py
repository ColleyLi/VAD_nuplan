...
def lidiar_render_navsim(sample_token, data, out_path=None, out_name=None, traj_use_perstep_offset=True):
    bbox_gt_list = []
    bbox_pred_list = []
    
    # cur_token = openscene_data[index]
    cur_token = "temp"
    ...
    # TODO 这里的 box 类是进行后续处理的关键 需要产生 gt_boxes 和 pred_boxes
    # 后续还要进行 box 内部的坐标转化 进行对应的平移旋转操作
   
    for i in num_boxes:
        # 在 nuscenes 里面 ann 是list 里面包含不同 box 的信息
        # 但是在 navsim 这里 ann 是一个 dict 想要进行 box 的循环就需要在 box 里面进行
        # content = nusc.get('sample_annotation', ann)
        
        # TODO 我需要重写下main的 get_gt_fut_trajs 函数
        gt_fut_trajs, gt_fut_masks = get_gt_fut_trajs_navsim(
            nusc=nusc, anno=content, cs_record=gt_boxes, 
            pose_record=gt_boxes, fut_ts=6
        )
        try:
            # 这里的 box 类也需要重写 人麻了
    ...
    visualize_sample_navsim(nusc, sample_token, gt_annotations, pred_annotations,
...
def boxes_to_sensor(boxes: List[EvalBox], cur_token: Dict):
    """
    ...
    for box in boxes:
        ...
        box.translate(-np.array(cur_token["ego2global_translation"]))
        box.rotate(Quaternion(cur_token['ego2global_rotation']).inverse)
        
        
        ## Move box to sensor coord system.
        box.translate(-np.array(cur_token['lidar2ego_translation']))
        box.rotate(Quaternion(cur_token['lidar2ego_rotation']).inverse)
...
def get_gt_fut_trajs_navsim(nusc: NuScenes,
                     anno,
                     cs_record,
                     pose_record,
                     fut_ts) -> None:
...
# 我看这个函数主要在利用的就是 pred_boxes 整体上和 gt_boxes 关系不大
def visualize_sample_navsim(
                            # nusc: NuScenes,
                            cur_token: Dict,
                            sample_token: str,
                            gt_boxes: EvalBoxes,
    ...
    boxes_gt = boxes_to_sensor(boxes_gt_global, cur_token)
    # Map EST boxes to lidar.
    boxes_est = boxes_to_sensor(boxes_est_global, cur_token)
    ...
    # Show Pred Map
    # TODO 我这里还不知道相应的 pred_data 是什么 以及我们是否能够得到它
...
def render_sample_data(
    ...
    """
    lidiar_render_navsim(sample_token, pred_data, out_path=out_path,
...
if __name__ == '__main__':
    ...
    for id in tqdm(range(len(sample_token_list))):
        ...
        ## 这里是在BEV 图像上增加相应的信息 (BEV str 和 相应的驾驶命令)
        ...
        vis_img = cv2.resize(pred_img, (800, 800))
