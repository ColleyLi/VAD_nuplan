...
def lidiar_render(sample_token, data, out_path=None, out_name=None, traj_use_perstep_offset=True):
    ...
    for ann in anns:
        ...
        # content.keys = (['token', 'sample_token', 'instance_token', 'visibility_token', 
        # 'attribute_tokens', 'translation', 'size', 'rotation', 
        # 'prev', 'next', 'num_lidar_pts', 'num_radar_pts', 'category_name'])
...
def visualize_sample(nusc: NuScenes,
    ...
    pred_ax.set_xlim(-30, 30)
    pred_ax.set_ylim(-30, 30)
    gt_ax.set_xlim(-30, 30)
    gt_ax.set_ylim(-30, 30)

    # Show Pred & GT Map
    result_dic = pred_data['map_results'][sample_token]['vectors']
    gt_vecs_pts_loc, gt_vecs_label = get_gt_vec_maps(sample_token)
    ...
    for vector in result_dic:
        ...
        pred_ax.plot(pts_x, pts_y, color=colors_plt[pred_label_3d],linewidth=1,alpha=0.8,zorder=-1)
        pred_ax.scatter(pts_x, pts_y, color=colors_plt[pred_label_3d],s=1,alpha=0.8,zorder=-1)  

    for index in range(gt_vecs_pts_loc.fixed_num_sampled_points.shape[0]):
        gt_pts_3d = gt_vecs_pts_loc.fixed_num_sampled_points[index]
        gt_label_3d = gt_vecs_label[index]
        pts_x = np.array([pt[0] for pt in gt_pts_3d])
        pts_y = np.array([pt[1] for pt in gt_pts_3d])

        gt_ax.plot(pts_x, pts_y, color=colors_plt[gt_label_3d],linewidth=1,alpha=0.8,zorder=-1)
        gt_ax.scatter(pts_x, pts_y, color=colors_plt[gt_label_3d],s=1,alpha=0.8,zorder=-1)  

    savepath = '/home/gyf/E2EDrivingScale/VAD/VAD-main/tools/analysis_tools'
    plt.savefig(savepath+'/map.png', bbox_inches='tight', dpi=200)
    ...
    for i, box in enumerate(boxes_est):
        ...
        if box.score < conf_th or abs(box.center[0]) > 15 or abs(box.center[1]) > 30:
            continue
        box.render(pred_ax, view=np.eye(4), colors=('tomato', 'tomato', 'tomato'), linewidth=1, box_idx=None)
        # if box.name in ['pedestrian']:
        #     continue
        if traj_use_perstep_offset:
            mode_idx = [0, 1, 2, 3, 4, 5]
            box.render_fut_trajs_grad_color(pred_ax, linewidth=1, mode_idx=mode_idx, fut_ts=6, cmap='autumn')
        else:
            box.render_fut_trajs_coords(pred_ax, color='tomato', linewidth=1)

    for i, box in enumerate(boxes_gt):
        if box.name in ignore_list:
            continue
        box.render(gt_ax, view=np.eye(4), colors=('tomato', 'tomato', 'tomato'), linewidth=1, box_idx=None)
        # if box.name in ['pedestrian']:
        #     continue
        if traj_use_perstep_offset:
            mode_idx = [0]
            box.render_fut_trajs_grad_color(gt_ax, linewidth=1, mode_idx=mode_idx, fut_ts=6, cmap='autumn')
        else:
            box.render_fut_trajs_coords(gt_ax, color='tomato', linewidth=1)

    plt.savefig(savepath+'/map+pred_box.png', bbox_inches='tight', dpi=200)

    # Show Pred & GT Planning.
    # in pred ax
    pred_ax.plot([-0.9, -0.9], [-2, 2], color='mediumseagreen', linewidth=1, alpha=0.8)
    pred_ax.plot([-0.9, 0.9], [2, 2], color='mediumseagreen', linewidth=1, alpha=0.8)
    pred_ax.plot([0.9, 0.9], [2, -2], color='mediumseagreen', linewidth=1, alpha=0.8)
    pred_ax.plot([0.9, -0.9], [-2, -2], color='mediumseagreen', linewidth=1, alpha=0.8)
    pred_ax.plot([0.0, 0.0], [0.0, 2], color='mediumseagreen', linewidth=1, alpha=0.8)
    # in gt ax
    gt_ax.plot([-0.9, -0.9], [-2, 2], color='mediumseagreen', linewidth=1, alpha=0.8)
    gt_ax.plot([-0.9, 0.9], [2, 2], color='mediumseagreen', linewidth=1, alpha=0.8)
    gt_ax.plot([0.9, 0.9], [2, -2], color='mediumseagreen', linewidth=1, alpha=0.8)
    gt_ax.plot([0.9, -0.9], [-2, -2], color='mediumseagreen', linewidth=1, alpha=0.8)
    gt_ax.plot([0.0, 0.0], [0.0, 2], color='mediumseagreen', linewidth=1, alpha=0.8)
    ...
    line_segments_gt = copy.deepcopy(line_segments)
    ...
    pred_ax.add_collection(line_segments)
    gt_ax.add_collection(line_segments_gt)

    plt.savefig(savepath+'/map+pred_box+planning.png', bbox_inches='tight', dpi=200)

    pred_ax.axes.xaxis.set_ticks([])
    pred_ax.axes.yaxis.set_ticks([])
    gt_ax.axes.xaxis.set_ticks([])
    gt_ax.axes.yaxis.set_ticks([])
...
if __name__ == '__main__':
    ...
    for id in tqdm(range(len(sample_token_list))): # len(sample_token_list)

        # out_path = save_path + '/' + str(id) + '/'
        # video_path = osp.join(out_path, 'vis.mp4')
        # video = cv2.VideoWriter(video_path, fourcc, 10, (2933, 800), True)
